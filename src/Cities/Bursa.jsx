...
function Bursa() {
  const [bursaHotels, setBursaHotels] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const bursaOtelListesi = response.data.$values.filter(
          (otel) => otel.sehirİsim === "Bursa"
        );
        setBursaHotels(bursaOtelListesi);
      })
