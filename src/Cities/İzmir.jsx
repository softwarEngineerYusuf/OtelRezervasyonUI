...
function İzmir() {
  const [izmirHotels, setİzmirHotels] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const izmirListesi = response.data.$values.filter(
          (otel) => otel.sehirİsim === "İzmir"
        );
        setİzmirHotels(izmirListesi);
      })
