...
function Muğla() {
  const [muğlaHotels, setMuğlaHotels] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const muğlaOtelListesi = response.data.$values.filter(
          (otel) => otel.sehirİsim === "Muğla"
        );
        setMuğlaHotels(muğlaOtelListesi);
      })
