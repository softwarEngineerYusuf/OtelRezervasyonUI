...
function Antalya() {
  const [antalyaHotels, setAntalyaHotels] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const antalyaOtelListesi = response.data.$values.filter(
          (otel) => otel.sehirİsim === "Antalya"
        );
        setAntalyaHotels(antalyaOtelListesi);
      })
