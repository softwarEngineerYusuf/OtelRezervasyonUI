...
function Mersin() {
  ...
  const [mersinHotels, setMersinHotels] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const mersinOtelListesi = response.data.$values.filter(
          (otel) => otel.sehirİsim === "Mersin"
        );
        setMersinHotels(mersinOtelListesi);
      })
