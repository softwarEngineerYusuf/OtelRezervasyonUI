...
function Aydın() {
  const [aydınHotels, setAydınHotels] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const aydınOtelListesi = response.data.$values.filter(
          (otel) => otel.sehirİsim === "Aydın"
        );
        setAydınHotels(aydınOtelListesi);
      })
