...
function GenelFiltre() {
  const [genelFiltreOtel, setGenelFiltreOtel] = useState([]);
  const [modalContent, setModalContent] = useState(null);
  const [inputSearch, setInputSearch] = useState("");

  const { selectedCity, selectedHotel, maxxPrice } = useFilterContext();

  const normalizeAndLowercase = (text) => {
    return text
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .toLowerCase();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://localhost:7082/api/Otel");
        const allOtels = response.data.$values;

        const formattedSelectedCity = normalizeAndLowercase(selectedCity);
        const formattedSelectedHotel = normalizeAndLowercase(selectedHotel);

        const filteredOtels = allOtels.filter((otel) => {
          const otelCity = normalizeAndLowercase(otel.sehirİsim);
          const otelName = normalizeAndLowercase(otel.isim);

          const isCityMatch = otelCity.includes(formattedSelectedCity);
          const isHotelMatch = otelName.includes(formattedSelectedHotel);
          const isPriceMatch = otel.tlFiyati <= maxxPrice;

          return isCityMatch && isHotelMatch && isPriceMatch;
        });

        setGenelFiltreOtel(filteredOtels);
      } catch (error) {
        console.error(error);
      }
    };
    ...
    fetchData();
  }, [selectedCity, selectedHotel, maxxPrice]);
  ...
  return (
    <section className="main container section">
      <div className="container">
        {genelFiltreOtel.length === 0 ? (
          <h1 style={{ color: "red" }} className="otel-bulunamadi">
            Otel Bulunamadı
          </h1>
        ) : (
          <div className="row">
            {genelFiltreOtel.map((hotel, index) => (
